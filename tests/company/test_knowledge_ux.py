...
def test_knowledge_overview_and_search_include_all_surfaces(tmp_path):
    ...
    assert overview["skills"][0]["last_used_at"]
    assert (
        overview["proposals"][0]["last_used_at"]
        == overview["proposals"][0]["created_at"]
    )
    assert (
        overview["pending_proposals"][0]["proposal_id"] == "skill-shared-release-notes"
    )
    exported = manager.export_markdown()
    assert "# Aider Plus Knowledge Export" in exported
    assert "## Playbooks" in exported
    assert "## Skills" in exported
    assert "Last Used:" in exported

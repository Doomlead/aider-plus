...
def run_onboarding(io: InputOutput | None = None) -> int:
    ...
    if config_exists:
        ...
        if not io.confirm_ask(
            "Do you want to continue and update onboarding settings?", default="y"
        ):
    ...
    provider_keys = [
        ...
        ("webhook_adapter_secret", "AIDER_WEBHOOK_ADAPTER_SECRET"),
    ]
    config: dict[str, str | bool | list[str]] = {"use_architect_mode": True}
    ...
    adapters = input(
        "Additional adapters to enable (comma-separated: discord, slack, webhook) []: "
    ).strip()
    config["adapters"] = [
        adapter.strip()
        for adapter in adapters.split(",")
        if adapter.strip() in {"discord", "slack", "webhook"}
    ]
    ...
    project_memory = (
        Path(config["default_repo"]).expanduser() / ".aider" / "project_memory.json"
    )
    project_memory.parent.mkdir(parents=True, exist_ok=True)
    if not project_memory.exists():
        project_memory.write_text(
            json.dumps({"initialized_by": "aider onboard"}, indent=2), encoding="utf-8"
        )

    config_path.write_text(json.dumps(config, indent=2), encoding="utf-8")
    io.tool_warning(f"Saved onboarding config to {config_path}.")
    io.tool_warning(
        "API keys are stored in plain text. Consider moving keys to environment variables."
    )

...
class KnowledgeManager:
    ...
    def get_all_skills(self) -> list[dict[str, Any]]:
        recent_by_key = {
            ...
            for item in self.get_recent_skills(limit=1000)
        }
        skills = []
        for skill in self.skill_manager.manager.list_skills():
            payload = self._skill_to_dict(skill)
            recent = recent_by_key.get((payload.get("scope"), payload.get("name")), {})
            payload["last_used_at"] = recent.get("last_used_at")
            payload["last_used_role"] = recent.get("role")
    ...
    def get_skill_proposals(self, *, status: str | None = None) -> list[dict[str, Any]]:
        ...
        for proposal in sorted(proposals, key=lambda p: p.created_at, reverse=True):
            ...
            payload["last_used_at"] = (
                proposal.metadata.get("last_used_at") or proposal.created_at
            )
    ...
    def export_markdown(self) -> str:
        """Return a portable Markdown snapshot of playbooks and approved skills."""

        playbooks = self.get_all_playbooks()
        skills = self.get_all_skills()
        lines = ["# Aider Plus Knowledge Export", ""]
        lines.extend(["## Playbooks", ""])
        if not playbooks:
            lines.extend(["_No playbook items have been learned yet._", ""])
        for item in playbooks:
            lines.extend(
                [
                    ...
                    f"- Timestamp: {item.get('created_at') or 'unknown'}",
                    "",
                    str(item.get("content") or "").strip() or "_No content._",
                    "",
                ]
            )
        ...
        lines.extend(["## Skills", ""])
        if not skills:
            lines.extend(["_No approved skills are available yet._", ""])
        for skill in skills:
            lines.extend(
                [
                    f"### {skill.get('scope')}/{skill.get('name')}",
                    "",
                    f"- Title: {skill.get('title') or 'Untitled'}",
                    f"- Last Used: {skill.get('last_used_at') or 'Never'}",
                    f"- Path: `{skill.get('path') or 'n/a'}`",
                    "",
                    str(
